...
JS + Leaflet inlined). A pre-built copy is also committed as `world-map.html` for
convenience (just open it in a browser).
...
world-map.html        # pre-built single-file output (generated by `pnpm build`)
```
...
- Country list grouped by continent (collapsible) or flat A–Z, searchable; zooms
  to and selects a country.
- Clicking a continent header highlights all its member countries on the map and
  shows aggregate facts (countries, population, area, GDP) in the panel.
